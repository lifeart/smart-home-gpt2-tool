...
# ]
# ///
"""Iter 22.3 — SFT GPT-2 124M v11 = pure data scaling (no Granite).

v11 dataset: v6 (20600) + HA (2021) + Nemotron (4958) = 27579 rows, all
single-variant (no Granite 3-way relabel).

Goal: isolate whether *data alone* (HA + Nemotron breadth on top of strong
v6 base) lifts metrics, vs. v9/v10 which mixed in Granite curriculum and
both regressed in-domain.

Hyperparams: 1 epoch, lr 1e-5, bs 8, seq_len 1024, bf16 (L40s).
Mirrors train_hf_v6.py except batch=8 (L40s has headroom; v9/v10 used bs 8).

Dataset: lifeart/smart-home-sft-v2 (sh_train_v11.json)
Base:    openai-community/gpt2
Target:  lifeart/smart-home-gpt2-v11

Run on HF Jobs:
    hf jobs uv run --flavor l40sx1 --secrets HF_TOKEN --timeout 1h \\
        training/train_hf_v11.py
"""
import json
import os
import time
...
TARGET_REPO = os.environ.get("TARGET_REPO", "lifeart/smart-home-gpt2-v11")
...
def main() -> None:
    ...
    api.upload_folder(
        ...
        repo_id=TARGET_REPO,
        commit_message=(
            "SFT GPT-2 124M v11 — pure data scaling (v6 + HA + Nemotron, "
            "NO Granite). 1 epoch, bf16, batch 8, lr 1e-5, seq 1024."
        ),
    )
