...
# ]
# ///
"""SFT GPT-2 124M v7 — v6 base + 3k ToolACE + 2k xLAM (v7-only stride) + 500 Glaive.

Hyperparam changes vs v6:
  - per_device_train_batch_size = 8 (up from 2)
  - gradient_accumulation_steps = 1 (down from 2 — effective batch stays 8 since L40s
    has 48GB VRAM and seq_len 1024 fits batch 8 comfortably)
  - num_train_epochs = 2 (up from 1)
  - bf16 = True (L40s supports it; v6 used fp16)
  - learning_rate = 1e-5 (unchanged)
  - PAD seq_len = 1024 (unchanged)

If batch 8 OOMs on L40s at seq 1024, retry path: drop to batch 4 + grad-accum=2.

Dataset: lifeart/smart-home-sft-v2 (sh_train_v7.json, ~25882 rows)
Base:    openai-community/gpt2
Target:  lifeart/smart-home-gpt2-v7

Run on HF Jobs:
    hf jobs uv run --flavor l40sx1 --secrets HF_TOKEN --timeout 2h \\
        training/train_hf_v7.py
"""
import json
import os
...
TARGET_REPO = os.environ.get("TARGET_REPO", "lifeart/smart-home-gpt2-v7")
DATA_REPO = "lifeart/smart-home-sft-v2"
DATA_FILE = "sh_train_v7.json"
PAD = 1024
...
def main() -> None:
    ...
    api.upload_folder(
        ...
        repo_id=TARGET_REPO,
        commit_message=(
            "SFT GPT-2 124M v7 — v6 + 3k ToolACE + 2k xLAM_v7 + 500 Glaive, "
