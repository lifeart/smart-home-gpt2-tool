...
# ]
# ///
"""SFT GPT-2 124M v10 — Iter 21 — selective Granite curriculum.

Reads `sh_train_v10.json` (~56k rows: SH×3 + external×1). Same hyperparams as
v9 (L40s bf16 batch 8 lr 1e-5 seq_len 1024) but 1 epoch since dataset is
smaller-and-richer (no global 3x relabel on external). Push to
lifeart/smart-home-gpt2-v10.

Run on HF Jobs:
    hf jobs uv run --flavor l40sx1 --secrets HF_TOKEN --timeout 1h \\
        -d training/train_hf_v10.py
"""
import json
import os
...
TARGET_REPO = os.environ.get("TARGET_REPO", "lifeart/smart-home-gpt2-v10")
DATA_REPO = "lifeart/smart-home-sft-v2"
DATA_FILE = os.environ.get("DATA_FILE", "sh_train_v10.json")
PAD = 1024
...
def main() -> None:
    ...
    api.upload_folder(
        ...
        repo_id=TARGET_REPO,
        commit_message=(
            "SFT GPT-2 124M v10 — selective Granite (SH×3, external×1), "
            "L40s bf16 batch 8 epochs 1"
        ),
    )
