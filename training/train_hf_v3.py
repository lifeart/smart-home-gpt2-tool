...
# ]
# ///
"""SFT GPT-2 124M v3 — v2 mix + Whisper-noise-augmented SH (3x repeat).

Dataset: lifeart/smart-home-sft-v2 (sh_train_v3.json, ~11.6k rows)
Base:    openai-community/gpt2 (fresh SFT pass, NOT v2 continuation)
Target:  lifeart/smart-home-gpt2-v3

Run on HF Jobs:
    hf jobs uv run --flavor t4-small --secrets HF_TOKEN --timeout 2h \\
        training/train_hf_v3.py
"""
import json
import os
...
TARGET_REPO = os.environ.get("TARGET_REPO", "lifeart/smart-home-gpt2-v3")
DATA_REPO = "lifeart/smart-home-sft-v2"
DATA_FILE = "sh_train_v3.json"
PAD = 1024
...
def main() -> None:
    ...
    api.upload_folder(
        ...
        repo_id=TARGET_REPO,
        commit_message=(
            "SFT GPT-2 124M v3 — v2 mix + Whisper-noise SH x3 (~11.6k)"
        ),
    )
