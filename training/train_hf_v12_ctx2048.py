...
# ]
# ///
"""Iter 34 — extend smart-home GPT-2 context window 1024 -> 2048.
...
1024 is a hard wall. This script lifts that wall:

  1. Load GPT2LMHeadModel (124M, 12 layers, 768 hidden).
  2. `extend_wpe(model, 2048)` — linear-interpolate the 1024x768 `wpe`
     table to 2048x768 and install it as a new nn.Embedding(2048, 768).
     `config.n_positions = config.n_ctx = 2048`.
  3. One short SFT epoch at seq_len 2048 so the model adapts to the
...
Hyperparams mirror train_hf_v11.py except for the 2048-ctx memory budget:
batch 8 -> 4, grad-accum 1 -> 2 (effective batch stays 8).

Dataset: lifeart/smart-home-sft-v2 (sh_train_v11.json) — same as v11.
Base:    openai-community/gpt2
Target:  lifeart/smart-home-gpt2-v12-ctx2048

Run on HF Jobs:
    hf jobs uv run --flavor l40sx1 --secrets HF_TOKEN --timeout 1h \\
        --detach training/train_hf_v12_ctx2048.py
"""
import json
import os
import time
...
TARGET_REPO = os.environ.get(
    "TARGET_REPO", "lifeart/smart-home-gpt2-v12-ctx2048"
)
DATA_REPO = "lifeart/smart-home-sft-v2"
DATA_FILE = "sh_train_v11.json"
NEW_CTX = 2048
PAD = 2048
...
def main() -> None:
    ...
    # sanity: forward a >1024-token batch before training
    with torch.no_grad():
        probe = torch.zeros((1, 1500), dtype=torch.long)
        _ = model(probe)
    print("[wpe] >1024-token forward probe OK (seq_len=1500)")
    # ------------------------------------------------------------------
    ...
    api.upload_folder(
        ...
        repo_id=TARGET_REPO,
        commit_message=(
            "Iter 34: GPT-2 124M v12 — context window extended 1024->2048 "
            "via linear-interpolated wpe, then 1 SFT epoch on sh_train_v11 "
            "(L40s bf16, batch 4 x grad-accum 2, lr 1e-5, seq 2048)."
        ),
    )
