...
# ]
# ///
"""SFT GPT-2 124M v5 — v4 base + HF-Inference synthetic (~21.5k rows).

Same hyperparams as v4: 1 epoch, lr 1e-5, bs 2, grad-accum 2, seq_len 1024.

Dataset: lifeart/smart-home-sft-v2 (sh_train_v5.json)
Base:    openai-community/gpt2
Target:  lifeart/smart-home-gpt2-v5

Run on HF Jobs:
    hf jobs uv run --flavor t4-small --secrets HF_TOKEN --timeout 2h30m \\
        training/train_hf_v5.py
"""
import json
import os
...
TARGET_REPO = os.environ.get("TARGET_REPO", "lifeart/smart-home-gpt2-v5")
DATA_REPO = "lifeart/smart-home-sft-v2"
DATA_FILE = "sh_train_v5.json"
PAD = 1024
...
def main() -> None:
    ...
    api.upload_folder(
        ...
        repo_id=TARGET_REPO,
        commit_message=(
            "SFT GPT-2 124M v5 — v4 base + HF-Inference Llama-3.3-70B synthetic (~21.5k)"
        ),
    )
