...
# ]
# ///
"""SFT GPT-2 124M v4 — v3 mix + xlam-sh-filtered + relaxed hermes +
twin contrastive + numeric variants. Noisy upsample x5.

Dataset: lifeart/smart-home-sft-v2 (sh_train_v4.json, ~18.5k rows)
Base:    openai-community/gpt2 (fresh SFT pass — not v3 continuation)
Target:  lifeart/smart-home-gpt2-v4

Run on HF Jobs:
    hf jobs uv run --flavor t4-small --secrets HF_TOKEN --timeout 2h \\
        training/train_hf_v4.py
"""
import json
import os
...
TARGET_REPO = os.environ.get("TARGET_REPO", "lifeart/smart-home-gpt2-v4")
DATA_REPO = "lifeart/smart-home-sft-v2"
DATA_FILE = "sh_train_v4.json"
PAD = 1024
...
def main() -> None:
    ...
    api.upload_folder(
        ...
        repo_id=TARGET_REPO,
        commit_message=(
            "SFT GPT-2 124M v4 — v3 mix + xlam-sh + hermes-relaxed + "
            "twin contrastive + numeric variants (~18.5k)"
        ),
    )
