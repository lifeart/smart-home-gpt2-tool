...
# ]
# ///
"""SFT GPT-2 124M v2 — bigger mix incl. irrelevance + IoT-aux examples.

Dataset: lifeart/smart-home-sft-v2 (sh_train_v2.json, ~8.6k rows)
Base:    openai-community/gpt2 (fresh SFT pass, not v1 continuation)
Target:  lifeart/smart-home-gpt2-v2

Run on HF Jobs:
    hf jobs uv run --flavor t4-small --secrets HF_TOKEN --timeout 2h \\
        training/train_hf_v2.py
"""
import json
import os
...
TARGET_REPO = os.environ.get("TARGET_REPO", "lifeart/smart-home-gpt2-v2")
DATA_REPO = "lifeart/smart-home-sft-v2"
DATA_FILE = "sh_train_v2.json"
PAD = 1024
...
def main() -> None:
    ...
    api.upload_folder(
        ...
        repo_id=TARGET_REPO,
        commit_message=(
            "SFT GPT-2 124M v2 — 8.6k mix: SH x5 + xlam-irrelevance + "
            "hermes-IoT"
        ),
    )
