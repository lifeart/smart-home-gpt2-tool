 ...
 * @returns {{names: string[], paramKeys: Map<string,string[]>}}
 */
export function buildSchemaConstraint(candidateNames, registry) {
  const paramKeys = new Map();
  for (const n of candidateNames) {
    const entry = registry[n];
    if (entry && entry.params) {
      paramKeys.set(n, Object.keys(entry.params));
    } else {
      // Unknown function — free-form arguments allowed.
      paramKeys.set(n, null);
    }
  }
  return { names: candidateNames, paramKeys };
}
...
export function isValidPrefix(s, constraint) {
  ...
  while (true) {
    ...
    i = skipWs(s, i);
    if (i === s.length) return true;
    // Parse a value (free-form). Returns updated i, or -1 if invalid, or
    // s.length+1 if still in-progress (need more chars).
